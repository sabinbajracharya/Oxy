...
Oxide exists because Rust's syntax is a joy but its borrow checker is a gatekeeper.

Millions of developers want sum types, pattern matching, traits, and expression-oriented code — but can't or won't invest the weeks it takes to internalize ownership, lifetimes, and borrowing. Oxide gives them the language they want without the fights they don't.
...
1. **Syntax fidelity** — if it's valid Rust syntax, it should be valid Oxide syntax. Deviations are bugs.

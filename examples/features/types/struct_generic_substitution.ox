...
// `Box<T> { value: T }` — accessing `.value` after `Box<i64>` should
// produce `i64`, not Unknown.
...
fn test_two_param_generic_ok() {
    let p: Pair<i64, String> = Pair { first: 1, second: "x".to_string() };
    assert_eq!(p.first, 1);
...
fn test_generic_struct_field_wrong_type_in_init() {
    // Box<i64> can't be initialized with a String value.
    let _b: Box<i64> = Box { value: "hello".to_string() };
}

#[compile_error]
fn test_generic_pair_field_wrong_type() {
    let _p: Pair<i64, String> = Pair { first: "wrong".to_string(), second: "ok".to_string() };
}

#[compile_error]
fn test_substituted_field_use_wrong_type() {
    // `b.value` is now `i64` (substituted from T), so assigning to a

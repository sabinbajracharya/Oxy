...
fn test_map_strings() {
    ...
    assert::eq(result[0], "HELLO");
    assert::eq(result[1], "WORLD");
}
...
fn test_filter_strings() {
    ...
    assert::eq(result[0], "bar");
    assert::eq(result[1], "baz");
}
...
fn test_rev_strings() {
    val v = ["a", "b", "c"];
    val result = v.iter().rev();
    assert::eq(result[0], "c");
    assert::eq(result[1], "b");
    assert::eq(result[2], "a");
}
...
fn test_enumerate_index_in_loop() {
    ...
    assert::eq(sum_indices, 3);   // 0+1+2
    assert::eq(sum_values, 600);  // 100+200+300
}
...
fn test_zip_sum_of_products() {
    ...
    assert::eq(total, 32);  // 1*4 + 2*5 + 3*6 = 4+10+18
}
...
fn test_flat_map_expand() {
    ...
    assert::eq(result, [1, 10, 2, 20, 3, 30]);
}
...
fn test_aoc_sum_of_doubled_evens() {
    ...
    assert::eq(result, 40);  // (2+4+6+8)*2 = 40
}
...
fn test_take_shares_source_state() {
    ...
    val it = v.iter();
    val t = it.take(2);
    assert::eq(t.next().unwrap(), 1);
    assert::eq(t.next().unwrap(), 2);
    assert::true(t.next().is_none());
    // Source advanced through `t`, so `it` resumes at element 3.
    assert::eq(it.next().unwrap(), 3);
    assert::eq(it.next().unwrap(), 4);
}

...
fn test_closure_capture_string() {
    ...
    val f = || string::format("{}, world", greeting);
    assert::eq(f(), "hello, world");
}
...
fn test_closure_returning_option() {
    ...
    assert::eq(safe_div(10, 2), Some(5));
    assert::eq(safe_div(10, 0), None);
}

...
fn test_match_with_side_effects_each_arm() {
    ...
    assert::eq(total, 1111);
}

// --- 18. match inside a fn returning value, called repeatedly ---
fn describe(s: Shape) -> String {
    match s {
        Shape::Circle(r) => string::format("circle r={}", r),
        Shape::Rect { w, h } => string::format("rect {}x{}", w, h),
...
fn test_match_fn_called_many_times() {
    assert::eq(describe(Shape::Circle(2.0)), "circle r=2.0");
    assert::eq(describe(Shape::Rect { w: 3.0, h: 4.0 }), "rect 3.0x4.0");
    assert::eq(describe(Shape::Triangle(1.0, 1.0, 1.0)), "tri");
    assert::eq(describe(Shape::Nothing), "nothing");
}

...
fn test_module_pub_fn() {
    assert::eq(math::add(2, 3), 5);
    assert::eq(math::sub(10, 4), 6);
}

#[test]
fn test_nested_module() {
    assert::eq(math::advanced::square(5), 25);
    assert::eq(math::advanced::cube(3), 27);
}
...
fn test_field_visibility_pub() {
    ...
    assert::eq(c.visible, 0);
    val c2 = hidden_fields::bump(c);
    assert::eq(c2.visible, 1);
    assert::eq(hidden_fields::count(c2), 1);
}
...
#[test]
fn test_pub_crate() {
    assert::eq(crate_only::shared(), 99);
    assert::eq(crate_only::calls_shared(), 99);
}

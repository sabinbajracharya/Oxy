...
fn main() {
    ...
    val cmd = args.positionals.get(0).unwrap().to_string();

    val open_result = db_open("todos.db");
    val handle = match open_result {
        Ok(h) => h,
        Err(e) => {
    ...
    val _ = db_exec(handle, "CREATE TABLE IF NOT EXISTS todos (
    ...
    if cmd == "add" {
        if args.positionals.len() < 2 {
            val _ = db_close(handle);
            die("usage: tug run -- add <text>");
            return;
        }
        val text = args.positionals.get(1).unwrap().to_string();
        val exec_result = db_exec(handle, "INSERT INTO todos (text) VALUES (?1)", [text]);
        val id = db_last_id(handle);
        match exec_result {
            Ok(_) => success("added todo #" + id.to_string()),
            Err(e) => fail("error: " + e),
        }
    } else if cmd == "list" {
        val show_all = args.flags.contains_key("all") || args.flags.contains_key("a");
        val sql = if show_all {
            "SELECT id, text, done FROM todos ORDER BY id"
        } else {
            "SELECT id, text, done FROM todos WHERE done = 0 ORDER BY id"
        };
        val query_result = db_query(handle, sql);
        match query_result {
            Ok(rows) => {
                if rows.len() == 0 {
                    info("no todos found");
        ...
        val id_str = args.positionals.get(1).unwrap().to_string();
        val id_result = id_str.parse_int();
        match id_result {
            Ok(id) => {
                val exec_result = db_exec(handle, "UPDATE todos SET done = 1 WHERE id = ?1", [id]);
                match exec_result {
                    Ok(rows) => {
                        if rows > 0 {
        ...
        match id_result {
            Ok(id) => {
                val exec_result = db_exec(handle, "DELETE FROM todos WHERE id = ?1", [id]);
                match exec_result {
                    Ok(rows) => {
                        if rows > 0 {

...
/// Convert a `std::io::Result<String>` into a Oxide `Result` value.
...
/// Convert a `std::io::Result<()>` into a Oxide `Result` value.
...
mod tests {
    ...
    fn test_fs_write_and_read_roundtrip() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let path = "{t}/oxide_test_roundtrip_9a1b.txt";
    let w = std::fs::write(path, "hello oxide");
...
"#
    ...
    fn test_fs_exists_true() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let path = "{t}/oxide_test_exists_7c2d.txt";
...
"#
    ...
    fn test_fs_exists_false() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    println!("{{}}", std::fs::exists("{t}/oxide_nonexistent_file_xyz_00.txt"));
}}
"#
        ));
        assert_eq!(out, "false\n");
    ...
    fn test_fs_is_file_and_is_dir() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let path = "{t}/oxide_test_isfile_3e4f.txt";
...
"#
    ...
    fn test_fs_create_dir_all() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let dir = "{t}/oxide_test_nested_c1d2/sub1/sub2";
    let c = std::fs::create_dir_all(dir);
    println!("{{}}", std::fs::is_dir(dir));
    let d = std::fs::remove_dir_all("{t}/oxide_test_nested_c1d2");
    println!("{{}}", std::fs::exists("{t}/oxide_test_nested_c1d2"));
}}
"#
        ));
        assert_eq!(out, "true\nfalse\n");
    ...
    fn test_fs_rename() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let src = "{t}/oxide_test_rename_src_5e6f.txt";
    let dst = "{t}/oxide_test_rename_dst_5e6f.txt";
...
"#
    ...
    fn test_fs_copy() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let src = "{t}/oxide_test_copy_src_7g8h.txt";
    let dst = "{t}/oxide_test_copy_dst_7g8h.txt";
...
"#
    ...
    fn test_fs_metadata_size() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let path = "{t}/oxide_test_meta_9i0j.txt";
...
"#
    ...
    fn test_fs_read_dir() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let dir = "{t}/oxide_test_readdir_k1l2";
    let c = std::fs::create_dir(dir);
    let w1 = std::fs::write("{t}/oxide_test_readdir_k1l2/a.txt", "a");
    let w2 = std::fs::write("{t}/oxide_test_readdir_k1l2/b.txt", "b");
...
"#
    ...
    fn test_fs_append() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let path = "{t}/oxide_test_append_m3n4.txt";
...
"#
    ...
    fn test_fs_remove_file() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    let path = "{t}/oxide_test_remove_o5p6.txt";

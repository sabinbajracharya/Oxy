//! Tree-walking interpreter for the Oxide language.
//!
...
impl Interpreter {
    ...
    fn register_module(&mut self, module: &ModuleDef) -> Result<(), FerriError> {
        let items = if let Some(body) = &module.body {
            ...
            // File-based module: load from `name.ox` or `name/mod.ox`
    ...
    fn load_module_file(&self, name: &str, span: Span) -> Result<String, FerriError> {
        ...
        // WHY: We try both `name.ox` and `name/mod.ox` to mirror Rust's module resolution
        // convention—a module can be either a single file (`foo.ox`) or a directory with an
        // entry point (`foo/mod.ox`). This lets users organise large modules into subdirectories
    ...
    fn eval_call_expr(
        ...
        // WHY: Oxide simulates async with lazy thunks (FutureData) instead of real Rust futures
    ...
    // Since Oxide is interpreted (no separate compilation phase), static exhaustiveness
...
/// Parse and execute a Oxide program from a file path with CLI args.
...
/// Discover and run `#[test]` functions in a Oxide source file.
...
mod tests {
    ...
    fn test_println_string() {
        let output = run_and_capture(r#"fn main() { println!("Hello, Oxide!"); }"#);
        assert_eq!(output, vec!["Hello, Oxide!\n"]);
    }
...
fn main() {
    let a = Article::new(String::from("Oxide"), String::from("A Rust-like language"));
    println!("{}", a.summarize());
}
"#,
        );
        assert_eq!(out, vec!["Oxide: A Rust-like language\n"]);
    }
...
"#;
        let result = run_file_with_args("test.ox", source, vec![]);
        assert!(result.is_err());
        let err = result.unwrap_err();
    ...
    fn test_test_runner_basic() {
        ...
        "#;
        let results = super::run_tests("test.ox", source).unwrap();

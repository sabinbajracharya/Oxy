...
use oxide_core::ast::{Item, Program};
use oxide_core::errors::FerriError;
...
impl OxideLsp {
    ...
    fn diagnose(source: &str) -> Vec<Diagnostic> {
        ...
        // Try lexing first
        if let Err(e) = oxide_core::lexer::tokenize(source) {
    ...
    fn try_parse(source: &str) -> Option<Program> {
        oxide_core::parser::parse(source).ok()
    }
...
fn error_to_diagnostic(e: &FerriError) -> Diagnostic {
    ...
    // Oxide spans are 1-indexed; LSP is 0-indexed.
...
impl LanguageServer for OxideLsp {
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client
            .log_message(MessageType::INFO, "Oxide LSP initialized")
            .await;
    }
...
fn item_definition_span(item: &Item, name: &str) -> Option<oxide_core::lexer::Span> {
...
fn item_hover_info(item: &Item, name: &str) -> Option<String> {
    ...
    match item {
        ...
        Item::Struct(s) => Some(format!("```oxide\nstruct {}\n```", s.name)),
        Item::Enum(e) => {
        ...
        Item::Trait(t) => Some(format!("```oxide\ntrait {}\n```", t.name)),
        _ => Some(format!("**{}**", name)),
    }
}
...
async fn main() {
    ...
    let (service, socket) = LspService::new(|client| OxideLsp {
        client,
...
mod tests {
    ...
    fn test_word_at_position() {
        ...
        let word = OxideLsp::word_at_position(source, Position::new(0, 3));
        assert_eq!(word, Some("hello".to_string()));

        let word = OxideLsp::word_at_position(source, Position::new(0, 0));
        assert_eq!(word, Some("fn".to_string()));

        let word = OxideLsp::word_at_position(source, Position::new(0, 12));
    ...
    fn test_diagnose_valid_source() {
        let diagnostics = OxideLsp::diagnose("fn main() {}");
        assert!(diagnostics.is_empty());
    }

    #[test]
    fn test_diagnose_invalid_source() {
        let diagnostics = OxideLsp::diagnose("fn {");
        assert!(!diagnostics.is_empty());
    }
    ...
    fn test_document_symbols() {
        ...
        let program = OxideLsp::try_parse(source).unwrap();

...
use oxide_core::errors::{CallFrame, FerriError};
use oxide_core::interpreter::Interpreter;
...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        Some("--version" | "-V") => {
            println!("{}", oxide_core::version_string());
        }
        Some("--help" | "-h") | None => {
            print_help();
        }
        ...
        Some(cmd) => {
            ...
            eprintln!("Run 'oxide --help' for usage information.");
            process::exit(2);
        }
    }
}
...
fn run_file(path: &str) {
    ...
    match oxide_core::interpreter::run_file_with_args(path, &source, program_args) {
        Ok(_) => {}
        Err(runtime_err) => {
...
fn run_repl() {
    ...
    let history_path = dirs_home().map(|h| format!("{}/.oxide_history", h));
    ...
    loop {
        ...
        match trimmed {
            ...
            ":help" | ":h" => {
                ...
                println!();
                println!("Enter Oxide code directly. Items (fn, struct, enum, impl,");
        ...
        if is_item {
            ...
            match oxide_core::parser::parse(&input) {
                Ok(program) => {
        ...
        match oxide_core::parser::parse(&wrapped) {
            Ok(program) => {
                if let Some(oxide_core::ast::Item::Function(f)) = program.items.first() {
                    for stmt in &f.body.stmts {
                        match interp.execute_stmt(stmt) {
                            Ok(val) => {
                                if val != oxide_core::types::Value::Unit {
                                    println!("{val}");
                                }
                            }
                            Err(e) => {
            ...
            Err(_) => {
                ...
                match oxide_core::parser::parse(trimmed) {
                    Ok(program) => {
                        for item in &program.items {
                            if let Err(e) = interp.register_item(item) {
                                display_error(&e, trimmed, &[]);
...
fn run_test_file(path: &str) {
    ...
    match oxide_core::interpreter::run_tests(path, &source) {
        Ok(results) => {
            let mut passed = 0;
            let mut failed = 0;
...
fn dump_tokens(path: &str) {
    ...
    match oxide_core::lexer::tokenize(&source) {
        Ok(tokens) => {
            for token in &tokens {
                println!(
                    "{:>4}:{:<3} {:?}",
...
fn dump_ast(path: &str) {
    ...
    match oxide_core::parser::parse(&source) {
        Ok(program) => {

...
fn compile_regex(pattern: &str, span: &Span) -> Result<regex::Regex, PipelineError> {
    regex::Regex::new(pattern)
        .map_err(|e| runtime_error(format!("invalid regex pattern `{pattern}`: {e}"), span))
}

/// Convert a regex `Match` into a Oxy struct with text, start, end fields.
    ...
    fn test_regex_captures_named() {
        let out = run(
            "fn main() {\n\
             ...
             io::println(\"{}\", caps.get(\"year\").unwrap());\n\
             io::println(\"{}\", caps.get(\"month\").unwrap());\n\
             }\n\
             }",
        );
        assert_eq!(out, "2024\n07\n");

        ...
        "read_dir" => {
            ...
            match std::fs::read_dir(path) {
                Ok(entries) => {
                    ...
                    Ok(Value::ok(Value::Vec(std::rc::Rc::new(
...
/// Convert a `std::io::Result<String>` into a Oxy `Result` value.
...
/// Convert a `std::io::Result<()>` into a Oxy `Result` value.
...
mod tests {
    ...
    fn test_fs_write_and_read_roundtrip() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    val path = "{t}/oxy_test_roundtrip_9a1b.txt";
...
"#
    ...
    fn test_fs_rename() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    val src = "{t}/oxy_test_rename_src_5e6f.txt";
...
"#
    ...
    fn test_fs_copy() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    val src = "{t}/oxy_test_copy_src_7g8h.txt";
    val dst = "{t}/oxy_test_copy_dst_7g8h.txt";
...
"#
    ...
    fn test_fs_metadata_size() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    val path = "{t}/oxy_test_meta_9i0j.txt";
...
"#
    ...
    fn test_fs_read_dir() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    val dir = "{t}/oxy_test_readdir_k1l2";
    val c = std::fs::create_dir(dir);
    val w1 = std::fs::write("{t}/oxy_test_readdir_k1l2/a.txt", "a");
...
"#
    ...
    fn test_fs_append() {
        ...
        let out = run(&format!(
            r#"
fn main() {{
    val path = "{t}/oxy_test_append_m3n4.txt";
    val w = std::fs::write(path, "hello");
